...
if ($mode == 'RANGE') {
    ...
    $data = $ln->getIntradayRangeByPageGroup($range_start, $range_end);

    // Get info to report
    $report = $data;
} else if ($mode == 'TIMESTAMP') {
    // yo lognormal, give me some data.
    $data = $ln->getIntradayRangeByPageGroup($timestamp);
    ...
    // yo lognormal, give me some data.
    $data = $ln->getIntradayRangeByPageGroup($day_start, $day_end);
...
foreach (array_keys($report) as $page) {
    foreach (array_keys($report[$page]) as $moment) {
        foreach ($report[$page][$moment] as $field => $value) {
            exec("echo $graph.pages.$page.$field $value $moment | nc $graphite_url $port\n");
        }
    }
}
